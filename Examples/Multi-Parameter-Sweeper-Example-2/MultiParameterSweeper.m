...
fprintf(fileid,'CHANNELBANDWIDTH_MHz,Tx_ANTENNA_COUNT,RX_ANTENNA_COUNT,INTER_ARRIVAL_TIME(micro sec),THROUGHPUT(Mbps),Data Packets transmitted,');
fclose(fileid);
...
for i = 1 : length(bandwidth)
    for j = 1:length(Tx_Antenna_count)
        for z = 1:length(Rx_Antenna_count)
            iat = ref_iat / ((bandwidth(i) / 10) * (Tx_Antenna_count(j) / 2));
            ...
            cmd="ConfigWriter.exe "...
                + string(bandwidth(i))...
                + " "...
                + string(Tx_Antenna_count(j))...
                + " "...
                + string(Rx_Antenna_count(z))...
                + " "...
                + string(iat);
            disp(cmd);
            system(cmd);
            ...
            if(isfile('IOPath\Metrics.xml'))
                ...
                if(fileid)
                    fprintf(fileid,'\n%d,%d,%d,%f,',bandwidth(i),Tx_Antenna_count(j),Rx_Antenna_count(z),iat);
                    fclose(fileid);
                end
                ...
                fileid=fopen('result.csv','a');
                if(fileid)
                    fprintf(fileid,'\n%d,%d,%d,%f,crash,',bandwidth(i),Tx_Antenna_count(j),Rx_Antenna_count(z),iat);
                    fclose(fileid);
                end
            end

            % Name of the Output folder to which the result will be saved
            OUTPUT_PATH = "Data\"+today+"\Output_"+string(bandwidth(i))+"_"+string(Tx_Antenna_count(j))+"_"+string(Rx_Antenna_count(z));
